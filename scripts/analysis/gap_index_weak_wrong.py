...
TEX_LOCAL_DIR = PROJECT_ROOT / "scripts" / "tables" / "gap_index"
TEX_NAME = "gap_index_classification_weak_wrong.tex"
...
CAPTION_WW = (
    r"\textbf{Per-weak-model ablation deficit profile on weak-wrong rows, "
    r"classification.} Companion to Table~\ref{tab:gap_index_classification} "
    r"restricted to rows where the weak model's predicted label is actually "
    r"incorrect — binary uses the per-dataset event-rate threshold "
    r"($P(y{=}1) > \mathrm{event\ rate}$), multi-class uses argmax. "
    r"Cell format is identical to Table~\ref{tab:gap_index_classification}: "
    r"$\mathrm{Strong}^{\%}_{\text{feat}}$, ranked within each row by "
    r"raw selection count."
)


def build_tex(gaps: dict, *, sort_by: str = "count") -> str:
    rank_headers = " & ".join(f"\\#{i+1}" for i in range(N_RANKS))
    col_spec = "l" + "l" * N_RANKS

    row_lines: list[str] = []
    for weak in WEAK_ORDER:
        bucket = gaps.get(weak)
        cells: list[str] = []
        if bucket and bucket["total_losses"] > 0:
            top = top_rows_for_weak(bucket, N_RANKS, sort_by=sort_by)
            for sm, fi, pct, _ in top:
                cells.append(f"\\gapcell{{{sm}}}{{{int(round(pct))}}}{{{fi}}}")
        while len(cells) < N_RANKS:
            cells.append("--")
        row_lines.append(f"{weak} & " + " & ".join(cells) + " \\\\")

    lines = [
        "% Generated by scripts/analysis/gap_index_weak_wrong.py",
        "% Weak-wrong subset: rows where the weak model's predicted label is wrong",
        "% (event-rate threshold for binary, argmax for multi-class). Cell macro",
        "% is shared with tables/gap_index_classification.tex — no need to redefine.",
        "\\providecommand{\\gapcell}[3]{{\\scriptsize\\ensuremath{\\text{#1}^{#2}_{#3}}}}",
        "\\begin{table*}[t]",
        "\\centering",
        "\\footnotesize",
        ...
        *row_lines,
        "\\bottomrule",
        "\\end{tabular}",
        f"\\caption{{{CAPTION_WW}}}",
        "\\label{tab:gap_index_classification_weak_wrong}",
        "\\end{table*}",
    ]
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    tex = build_tex(gaps_ww, sort_by="count")
    for path in (TEX_LOCAL_DIR / TEX_NAME, paper_table_path(TEX_NAME)):
        path.write_text(tex)
        print(f"Wrote {path}")
