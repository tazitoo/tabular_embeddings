#!/usr/bin/env python3
"""REBUTTAL (symmetric full-test-set) — vendored copy of ablation_sweep.py.

Verbatim copy of scripts/intervention/ablation_sweep.py with a single added
`--reverse` flag. The canonical paper script is left untouched so the
submitted numbers cannot be perturbed.

Forward (reverse=False, opt-in via --forward): identical to the paper —
ablate the overall-STRONGER model's unique concepts on the rows it wins
per-row (below-diagonal points collapse to the 45-degree line).

Reverse (reverse=True, DEFAULT): the rebuttal experiment. Ablate the overall-WEAKER
model's unique concepts on the rows *it* wins per-row (the previously
untouched above-diagonal points). Implemented by swapping the strong/weak
role assignment; because the entire pipeline is expressed as strong=donor /
weak=recipient / strong_wins=donor-wins-per-row, that one swap flips the row
filter to weak_wins, the unmatched lookup to the weak model's unique
concepts, and the tail to the recipient. Nothing else changes, so the greedy
is byte-for-byte the paper's.

Combine reverse output with the canonical forward output/ablation_sweep to
get full-test-set coverage. Answers reviewer dVDs ("report over the full test
...
Usage:
    ...
    python -m scripts.rebuttal.ablation_sweep_symmetric --models mitra carte --device cuda
    # forward reproduction check against the canonical paper output:
    python -m scripts.rebuttal.ablation_sweep_symmetric --models mitra carte --forward --device cuda
"""
import argparse
import json
import logging
...
def run_dataset(
    ...
    reverse=True (default here — the rebuttal experiment): swap the
    strong/weak role assignment so the ablation donor becomes the
    overall-WEAKER model and the row filter becomes the rows *it* wins
    per-row (the above-diagonal points). Every downstream step is unchanged
    because it is written in terms of strong=donor / weak=recipient /
    strong_wins=donor-wins-per-row.

    reverse=False reproduces the paper's forward (below-diagonal) direction
    exactly; used only by the --forward validation check.
    """
    ...
    # to (N,2) so step_preds / strong/weak preds align with intervention
    # output shape. Without this, mitra-as-strong ablations crash at
    # step_preds assignment when intervention returns (N,2).
    def _widen_binary(p):
    ...
    if reverse:
        # Rebuttal: make the ablation donor the overall-WEAKER model, so the
        # row filter (strong_loss < weak_loss) selects the rows the weaker
        # model wins per-row (above-diagonal points). Everything downstream
        # is expressed via strong=donor / weak=recipient and follows.
    ...
    return {
        "strong_model": strong,
        "weak_model": weak,
        # Rebuttal metadata: in reverse mode strong=donor is the overall-weaker
        # model and weak=recipient is the overall-stronger model.
        "reverse": bool(reverse),
        "donor_model": strong,
        "recipient_model": weak,
        "optimal_k": optimal_k,
...
def main():
    ...
    parser.add_argument("--forward", action="store_true",
                        help="Run the paper's forward (below-diagonal) direction "
                             "instead of the default reverse (above-diagonal) "
