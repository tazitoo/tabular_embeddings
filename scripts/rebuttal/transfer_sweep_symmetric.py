#!/usr/bin/env python3
"""REBUTTAL (symmetric full-test-set) — vendored copy of transfer_sweep_v2.py.

Verbatim copy of scripts/intervention/transfer_sweep_v2.py with a single added
`--reverse` flag (default True here). The canonical paper script is left
untouched so the submitted numbers cannot be perturbed.

Forward (reverse=False): identical to the paper — transfer the overall-STRONGER
model's unique concepts into the overall-WEAKER model on rows the stronger
model wins per-row (below-diagonal points move up to the 45-degree line).

Reverse (reverse=True, default): the rebuttal experiment. Transfer the
overall-WEAKER model's unique concepts into the overall-STRONGER model on the
rows the weaker model wins per-row (above-diagonal points move right to the
line). This is the weak->strong direction reviewer SD9t explicitly asked about
("does the strong model have all the weaker model's concepts?"). Implemented by
swapping the strong/weak role assignment; because the pipeline is expressed as
strong=donor / weak=recipient / strong_wins=donor-wins-per-row, that one swap
flips the whole direction. The greedy is byte-for-byte the paper's.

Combine reverse output with the canonical forward output/transfer_sweep_v2 to
get full-test-set coverage (answers dVDs) and the reverse direction (answers
SD9t).
...
def run_dataset(
    ...
    reverse=True (default here — the rebuttal experiment): swap the strong/weak
    role assignment so the transfer donor becomes the overall-WEAKER model and
    the recipient the overall-STRONGER model, restricted to the rows the weaker
    model wins per-row (above-diagonal points). This is the weak->strong
    direction SD9t asked about. reverse=False reproduces the paper's forward
    direction exactly (used by the --forward validation check).
    ...
    if reverse:
        # Rebuttal: make the transfer donor the overall-WEAKER model and the
        # recipient the overall-STRONGER model, so the row filter
        # (strong_loss < weak_loss) selects the rows the weaker model wins
        # per-row (above-diagonal points). weak->strong direction.
...
def main():
    ...
    parser.add_argument("--forward", action="store_true",
                        help="Run the paper's forward (below-diagonal, strong->weak) "
                             "direction instead of the default reverse "
                             "(above-diagonal, weak->strong) rebuttal direction. "
                             "Used to validate this vendored copy reproduces "
                             "canonical output/transfer_sweep_v2.")
    ...
    logger.info(f"Transfer sweep [{'REVERSE/above-diagonal weak->strong' if reverse else 'FORWARD/below-diagonal strong->weak'}]: "
