...
def main():
    ...
    fig_dir = PROJECT_ROOT / "output" / "figures" / args.dataset
    ...
    layer_a = get_extraction_layer(model_a)
    layer_b = get_extraction_layer(model_b)
    ...
    sae_source, _ = load_sae(source_model, device=args.device)
    sae_target, _ = load_sae(target_model, device=args.device)
    ...
    print(f"  {disp_t}+vnode AUC = {auc_transferred:.4f}")
    if abs(gap) > 0.001:
