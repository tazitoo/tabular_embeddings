#!/usr/bin/env python3
"""
fig:dictionary_comparison — Pairwise concept overlap across all 7 TFMs.

Section 1 (Introduction). Heatmaps of Jaccard similarity between SAE
dictionaries at each Matryoshka scale band, computed by clustering features
across models by activation correlation.
...
def make_figure(
    ...
    cmap = plt.cm.YlOrRd
    vmin, vmax = 0.0, 0.55  # accommodate highest S1 values (~0.53)
...
def main():
    ...
    parser.add_argument("--output", type=str, default=None,
                        help="Output figure path (auto-named if not specified)")
    parser.add_argument("--task-filter", type=str, default="classification",
                        choices=["classification", "regression"],
                        help="Task type to filter datasets (default: classification)")
    ...
    if args.output is None:
        args.output = f"output/paper_figures/sec1/fig_dictionary_comparison_{task}.pdf"

    base_sae = sae_sweep_dir()
    base_emb = PROJECT_ROOT / "output" / "embeddings" / "tabarena"

    # Resolve models for the requested task
    model_list, task_filters = get_models_for_task(task)
    model_configs = []
    for display_name, sweep_dir, emb_dir_name in model_list:
    ...
    print(f"Task: {task}, Models: {model_names}")

    # Common datasets
    emb_dirs = {name: emb_dir for name, _, emb_dir in model_configs}
    common_datasets = find_common_datasets(emb_dirs)

    # Filter by task type: restrict to datasets matching the requested task
    dataset_tasks = get_dataset_tasks()
    required_tasks = {task_filters[name]
                      for name, _, _ in model_configs
                      if name in task_filters}
    # If no model-specific filter, use the global task filter
    if not required_tasks:
        required_tasks = {task}
    common_datasets = [ds for ds in common_datasets
                       if dataset_tasks.get(ds) in required_tasks]
    print(f"Task filter → {len(common_datasets)} datasets (tasks: {required_tasks})")
    ...
    # Save pairwise Jaccard data for downstream scripts (e.g. sec4 geometric_vs_concept)
    data_dir = PROJECT_ROOT / "output" / "paper_data" / "sec1"
    data_dir.mkdir(parents=True, exist_ok=True)
    jaccard_data = {"model_names": model_names, "task": task,
                    "n_datasets": len(loaded_datasets),
                    "bands": {}, "overall": {}}
    for bi, presence in enumerate(all_presence):
        sim = pairwise_jaccard(presence)
        band_key = BAND_LABELS[bi] if bi < len(BAND_LABELS) else f"band_{bi}"
        band_pairs = {}
        for i in range(len(model_names)):
            for j in range(i + 1, len(model_names)):
                pair_key = f"{model_names[i]}--{model_names[j]}"
                band_pairs[pair_key] = round(float(sim[i, j]), 4)
        jaccard_data["bands"][band_key] = band_pairs
    # Overall
    overall_presence = np.vstack([p for p in all_presence if p.shape[0] > 0])
    overall_sim = pairwise_jaccard(overall_presence)
    for i in range(len(model_names)):
        for j in range(i + 1, len(model_names)):
            pair_key = f"{model_names[i]}--{model_names[j]}"
            jaccard_data["overall"][pair_key] = round(float(overall_sim[i, j]), 4)
    data_path = data_dir / f"pairwise_jaccard_{task}.json"
