...
- **SAE training uses per-dataset StandardScaler + b_dec subtraction (round 6).** Embeddings must be normalized per-dataset before training: `(x - mean_d) / std_d` with stats computed from the train split. `build_sae_training_data.py` saves stats to `_norm_stats.npz`. The SAE's `encode()` subtracts `b_dec` from the input (standard formulation). Do NOT pass raw/un-normalized embeddings to the SAE — apply per-dataset normalization first. BatchNorm is deprecated.
